...
class AccountOpeningDocumentExport implements FromCollection, WithHeadings, WithMapping
{
    ...
    public function map($doc): array
    {
        return [
            ...
            $doc->barcode,
            $doc->status >= '3' ? $doc->dispatch->awb_pod : '-',
            $doc->status >= '3' ? $doc->dispatch->courierName->name : '-',
            $doc->status >= '3' ? $doc->dispatch->dispatch_date : '-',
            $doc->status >= '3' ? ($doc->dispatch->status >= '4' ? $doc->dispatch->dispatcher->first_name : '-'): '-',
            $doc->status >= '3' ? $doc->dispatch->courier_received_date : '-',
            $doc->status >= '3' ? ($doc->dispatch->status == '12' ? $doc->modifier->first_name : '-'): '-',
            $doc->status >= '3' ? $doc->dispatch->statusName->name : '-',
            $doc->status >= '3' ? $doc->dispatch->comments : '-',
            $doc->lot_no,
