...
class LoanDocumentExport implements FromCollection, WithHeadings, WithMapping
{
    ...
    public function map($doc): array
    {
        return [
            ...
            $doc->status >= 4 ? (optional($doc->dispatch)->status >= 4 ? optional($doc->dispatch->dispatcher)->first_name . ' (' . optional($doc->dispatch->dispatcher)->employee_id . ')' : '-') : '-',
            $doc->status >= 4 ? (optional($doc->getReceivedDate)->created_at ? date('d-m-Y', strtotime($doc->getReceivedDate->created_at)) : '-') : '-',
            $doc->status >= 4 ? (optional($doc->dispatch)->status == 12 ? optional($doc->dispatch->modifier)->first_name . '(' . optional($doc->dispatch->modifier)->employee_id . ')' : '-') : '-',
            $doc->status >= 4 ? optional(optional($doc->dispatch)->statusName)->name : '-',
            $doc->status >= 4 ? optional($doc->dispatch)->comments : '-',
            $doc->lot_no,
