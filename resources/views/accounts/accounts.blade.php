                                ...
                                @if ($loan_document)
                                    @foreach ($loan_document as $row)
                                        <tr>
                                            @hasrole('master|super_admin|admin')
                                                {{-- @if ($type != 'rejected') --}}
                                                    <td><input type="checkbox" class="loan @if(!in_array($row->status, [1,6])) d-none @endif" name="loan_ids[]" data-id="{{ bin2hex($row->encrypted_id) }}"></td>
                                                {{-- @endif --}}
                                            @elsehasanyrole('bo-maker|bo-checker')
                                                @if (in_array($type, ['pending', 'all', 'rejected']))
                                                    <td><input type="checkbox" class="loan @if(!in_array($row->status, [1,6])) d-none @endif" name="loan_ids[]" data-id="{{ bin2hex($row->encrypted_id) }}"></td>
                                            ...
                                            @elsehasrole('ro-supervisor')
                                                ...
                                                <td><input type="checkbox" class="loan" name="loan_ids[]" data-id="{{ bin2hex($row->encrypted_id) }}"></td>                                                {{-- @endif --}}
                                            @endhasrole
                                            ...
                                            @if ($type === 'received' && $row->status == 1)
                                                ...
                                                @endif
                                            @endrole --}}
                                            <td><a href="{{ URL::signedRoute('document.history',['id' => bin2hex($row->encrypted_id),'type' => $type,'dtype' => 'loan']) }}">{{ $row->unique_ref_no }}</a></td>
                                            {{-- <td><a href="{{ route('document.history',['id' => bin2hex($row->encrypted_id),'type' => $type,'dtype' => 'loan'])) }}">{{ $row->unique_ref_no }}</a></td> --}}                                            @unless(auth()->user()->hasAnyRole(['bo-maker', 'bo-checker', 'branch-user']))
                                            <td>{{ $row->region }}</td>
                                ...
                                @if ($gold_loan_document)
                                    @foreach ($gold_loan_document as $row)
                                        <tr>
                                            @hasrole('master|super_admin|admin')
                                                {{-- @if ($type !== 'rejected') --}}
                                                    <td><input type="checkbox" class="goldloan @if(!in_array($row->status, [1,6])) d-none @endif" name="goldloan_ids[]" data-id="{{ bin2hex($row->encrypted_id) }}"></td>
                                                {{-- @endif --}}
                                            @elsehasanyrole('bo-maker|bo-checker')
                                                @if (in_array($type, ['pending', 'all', 'rejected']))
                                                    <td><input type="checkbox" class="goldloan @if(!in_array($row->status, [1,6])) d-none @endif" name="goldloan_ids[]" data-id="{{ bin2hex($row->encrypted_id) }}"></td>
                                            ...
                                            @elsehasrole('ro-supervisor')
                                                {{-- @if ($type === 'received') --}}
                                                    <td><input type="checkbox" class="goldloan" name="goldloan_ids[]" data-id="{{ bin2hex($row->encrypted_id) }}"></td>                                                {{-- @endif --}}
                                            @endhasrole
                                            {{-- @hasanyrole('master|bo-maker|bo-checker')
                                            <td>
                                                @if ($row->status == 1 && !in_array($type, ['received', 'rejected']))
                                            ...
                                            @if ($type === 'received' && $row->status == 1)
                                                ...
                                                @endif
                                            @endrole --}}
                                            <td><a href="{{ URL::signedRoute('document.history',['id' => bin2hex($row->encrypted_id),'type' => $type,'dtype' => 'goldloan']) }}">{{ $row->unique_ref_no }}</a></td>
                                            {{-- <td><a href="{{ route('document.history',['id' => bin2hex($row->encrypted_id),'type' => $type,'dtype' => 'goldloan'])) }}">{{ $row->unique_ref_no }}</a></td> --}}                                            @unless(auth()->user()->hasAnyRole(['bo-maker', 'bo-checker', 'branch-user']))
                                            <td>{{ $row->region }}</td>
                                            <td>{{ $row->branch_name }}</td>
                                            @endunless
                                            <td>{{ $row->branch_code }}</td>
                                ...
                                @if ($account_opening_document)
                                    @foreach ($account_opening_document as $row)
                                        <tr>
                                            @hasrole('master|super_admin|admin')
                                                {{-- @if ($type !== 'rejected') --}}
                                                    <td><input type="checkbox" class="aof @if(!in_array($row->status, [1,6])) d-none @endif" name="aof_ids[]" data-id="{{ bin2hex($row->encrypted_id) }}"></td>
                                                {{-- @endif --}}
                                            @elsehasanyrole('bo-maker|bo-checker')
                                                @if (in_array($type, ['pending', 'all', 'rejected']))
                                                    <td><input type="checkbox" class="aof @if(!in_array($row->status, [1,6])) d-none @endif" name="aof_ids[]" data-id="{{ bin2hex($row->encrypted_id) }}"></td>
                                            ...
                                            @elsehasrole('ro-supervisor')
                                                {{-- @if ($type === 'received') --}}
                                                    <td><input type="checkbox" class="aof" name="aof_ids[]" data-id="{{ bin2hex($row->encrypted_id) }}"></td>                                                {{-- @endif --}}
                                            @endhasrole
                                            {{-- @hasanyrole('master|bo-maker|bo-checker')
                                            <td>
                                                @if ($row->status == 1 && !in_array($type, ['received', 'rejected']))
                                            ...
                                            @if ($type === 'received' && $row->status == 1)
                                                ...
                                                @endif
                                            @endrole --}}
                                            <td><a href="{{ URL::signedRoute('document.history',['id' => bin2hex($row->encrypted_id),'type' => $type,'dtype' => 'aof']) }}">{{ $row->unique_ref_no }}</a></td>
                                            {{-- <td><a href="{{ route('document.history',['id' => bin2hex($row->encrypted_id),'type' => $type,'dtype' => 'aof'])) }}">{{ $row->unique_ref_no }}</a></td> --}}                                            @unless(auth()->user()->hasAnyRole(['bo-maker', 'bo-checker', 'branch-user']))
                                            <td>{{ $row->region }}</td>
                                            <td>{{ $row->branch_name }}</td>
                                            @endunless
                                            <td>{{ $row->branch_code }}</td>
                                ...
                                @if ($dtrf_document)
                                    @foreach ($dtrf_document as $row)
                                        <tr>
                                            @hasrole('master|super_admin|admin')
                                                {{-- @if ($type !== 'rejected') --}}
                                                    <td><input type="checkbox" class="dtrf @if(!in_array($row->status, [1,6])) d-none @endif" name="dtrf_ids[]" data-id="{{ bin2hex($row->encrypted_id) }}"></td>
                                                {{-- @endif --}}
                                            @elsehasanyrole('bo-maker|bo-checker')
                                                @if (in_array($type, ['pending', 'all', 'rejected']))
                                                    <td><input type="checkbox" class="dtrf @if(!in_array($row->status, [1,6])) d-none @endif" name="dtrf_ids[]" data-id="{{ bin2hex($row->encrypted_id) }}"></td>
                                            ...
                                            @elsehasrole('ro-supervisor')
                                                {{-- @if ($type === 'received') --}}
                                                    <td><input type="checkbox" class="dtrf" name="dtrf_ids[]" data-id="{{ bin2hex($row->encrypted_id) }}"></td>                                                {{-- @endif --}}
                                            @endhasrole
                                            {{-- @hasanyrole('master|bo-maker|bo-checker')
                                            <td>
                                                @if ($row->status == 1 && !in_array($type, ['received', 'rejected']))
                                            ...
                                            @if ($type === 'received' && $row->status == 1)
                                                ...
                                                @endif
                                            @endrole --}}
                                            <td><a href="{{ URL::signedRoute('document.history',['id' => bin2hex($row->encrypted_id),'type' => $type,'dtype' => 'dtrf']) }}">{{ $row->unique_ref_no }}</a></td>
                                            {{-- <td><a href="{{ route('document.history',['id' => bin2hex($row->encrypted_id),'type' => $type,'dtype' => 'dtrf'])) }}">{{ $row->unique_ref_no }}</a></td> --}}                                            @unless(auth()->user()->hasAnyRole(['bo-maker', 'bo-checker', 'branch-user']))
                                            <td>{{ $row->region }}</td>
                                            <td>{{ $row->branch_name }}</td>
                                            @endunless
                                            <td>{{ $row->branch_code }}</td>
