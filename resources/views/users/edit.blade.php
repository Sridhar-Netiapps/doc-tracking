        ...
        <form action="{{ route('users.update', $user) }}" method="POST" id="userform">
            ...
            <div class="form-card row">
                ...
                <div class="col-4 mb-4 form-group">
                    ...
                    <input type="text" class="form-control @error('region') is-invalid @enderror" id="region" name="region" value="{{ old('region', $user->region) }}" readonly>
                    @error('region')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                <div class="col-4 mb-4 form-group">
                    ...
                    <input type="text" class="form-control @error('branch_id') is-invalid @enderror" id="branch_id" name="branch_id" value="{{ old('branch_id', $user->branch_id) }}" readonly>
                    @error('branch_id')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                <div class="col-4 mb-4 form-group">
                    ...
                    <input type="text" class="form-control @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email', $user->email) }}" readonly>
                    @error('email')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="col-4 mb-4 form-group">
                    ...
                    <input type="text" class="form-control @error('mobile_number') is-invalid @enderror" id="mobile_number" name="mobile_number" value="{{ old('mobile_number', $user->mobile_number) }}" readonly>
                    @error('mobile_number')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="col-4 mb-4 form-group">
                    ...
                    <input type="text" readonly class="form-control datepicker @error('dor') is-invalid @enderror" id="dor" name="dor" value="{{ old('dor', $user->dor) }}">
                    @error('dor')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <h2 class="mt-2">Assign Roles {{ $user->roles->value('name')}}</h2>
                <div class="h-100 align-items-start align-content-lg-stretch">
                            ...
                            <div class="col-6 form-group">
                                ...
                                <select name="role" class="form-control" required>
                                    ...
                                    @foreach ($roles as $role)
                                        @if ($role->name !== 'master')
                                            <option value="{{ $role->name }}" {{($user->roles->value('name') == $role->name) ? 'selected':''}}>
                                                {{-- {{ $role->name }} --}}
                ...
                <div class="form-check form-check-inline ms-5">
                  <input class="form-check-input" type="checkbox" name="module_role" value="doc" id="inlineCheckbox1" {{ (old('module_role', $user->module_role) == 'doc') ? 'checked':''}}>
                ...
                <div class="form-check form-check-inline ms-5">
                  ...
                  {{ (old('module_role', $user->module_role) == 'ins') ? 'checked':''}}>
                  <label class="form-check-label" for="inlineCheckbox2">Insurance</label>
                </div>
                <div class="form-check form-check-inline ms-5">
                  <input class="form-check-input" type="checkbox" name="module_role" value="doc_ins" id="inlineCheckbox3" {{ (old('module_role', $user->module_role) == 'doc_ins') ? 'checked':''}}>
